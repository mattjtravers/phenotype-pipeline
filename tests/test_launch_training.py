...
The CLI module is ``src/phenotype_pipeline/launch_training.py``; it is a thin
wrapper around ``phenotype_pipeline.deployment.launch_training_job`` that
...
from phenotype_pipeline import launch_training
...
def test_cli_emits_run_id_to_stdout_on_completion(capsys, monkeypatch):
    ...
    with patch("phenotype_pipeline.launch_training.launch_training_job") as mock_lib:
...
def test_cli_exits_nonzero_on_training_failure(capsys):
    with patch("phenotype_pipeline.launch_training.launch_training_job",
...
def test_cli_exits_nonzero_on_unreachable_bucket(capsys):
    ...
    with patch("phenotype_pipeline.launch_training.launch_training_job", side_effect=err):
